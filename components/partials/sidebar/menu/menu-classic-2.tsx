...
export function MenuClassic({}) {
  ...
  const hasAtLeastOnePermission = (menuLists: any) => {
    let permissionFound = false;

    menuLists.forEach(({ permission }: any) => {
      ...
      <ScrollArea className="[&>div>div[style]]:!block" dir={direction}>
        ...
        <nav className="mt-8 h-full w-full">
          <ul className="h-full flex flex-col min-h-[calc(100vh-48px-36px-16px-32px)] lg:min-h-[calc(100vh-32px-40px-32px)] items-start space-y-1 px-4">
            {menuList?.map(
              (
                { href, label, icon, active, id, submenus, permission },
                index
              ) =>
                hasPermission(roles, permission) && (
                  <li key={id} className="w-full mb-2 last:mb-0">
                    {submenus.length === 0 ? (
