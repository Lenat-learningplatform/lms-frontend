...
const unknownCategoryColors = ["purple", "pink", "indigo", "amber", "emerald"];
...
const CalendarView = () => {
  ...
  const { data: events } = useQuery({
    queryKey: ["calendar", dateRange, classId],
    queryFn: async () => {
      const params = {
        ...
        (category, index) => {
          ...
          const color =
            DefaultCategoryColors[
              category as keyof typeof DefaultCategoryColors
            ];
          const unknownColor =
            UNKNOWN_CATEGORY_COLORS[index % UNKNOWN_CATEGORY_COLORS.length];
          const unknownColorClass =
            unknownCategoryColors[index % unknownCategoryColors.length];

          return {
            label: category.charAt(0).toUpperCase() + category.slice(1),
            value: category,
            className: knownColor || unknownColor,
            color: color || unknownColorClass,
          };
        }
      );
  ...
  // Format date for the upcoming events list
          ...
          {categories.length > 0 &&
            categories.map((category) => {
              const categoryEvents = events
                ?.filter(
                  (event: any) =>
                    event.extendedProps.calendar === category.value
                )
                .slice(0, 2);
                  ...
                  <ul className="space-y-4 px-6">
                    {categoryEvents && categoryEvents.length > 0 ? (
                      categoryEvents.map((event: any) => (
                        <li
                          key={event.id}
                          className={`flex flex-col border-l-4 pl-4 border-${category.color}`}
                        >
