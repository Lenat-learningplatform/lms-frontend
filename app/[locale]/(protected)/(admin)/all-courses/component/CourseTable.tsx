...
const CourseTable: React.FC<Props> = ({ setEdit, setDeleteModal }) => {
  ...
  const updateCourse = useHasPermission("update_module");
  ...
  const {
    ...
    queryFn: async (): Promise<PaginatedCourse> => {
      const response = await api.get("modules", { params: { page } });
        ...
        return (
          <Select onValueChange={(value) => handleCourseAction(value, zCourse)}>
            ...
            <SelectContent>
              ...
              {updateCourse && <SelectItem value="edit">✏️ Edit</SelectItem>}
              {deleteCourse && (
  ...
  // Use modules.data as the table data when the query is successful
